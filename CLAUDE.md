...
当前实现范围：SPEC-0001 基础增量 + SPEC-0003-A 生命周期 + SPEC-0003-A2 执行隔离。存储 schema 2，wire 1.0，事件 schemaVersion 1。0003-B/C（保留/GC/归档/选路）未实现。仓库已初始化 Git，远程为 `git@github.com:GfM-rvP9UN6UDe3W/Multi-Agent.git`；尚未发布 npm/PyPI 包。
...
npm test                       # node:test，157 项，约 2 秒
npm run test:python            # unittest，40 项，约 4 秒
```
...
**状态变更一律进 `store.transaction()`，并在同一事务写事件。** A/Q/R 计数每次都从 dispatches 行重算（`scheduler()`），不维护内存计数器——改调度逻辑时不要引入缓存计数。
