...
1. A release pull request sets the version and names it in the changelog, and is merged into `main`:
   ...
   - It also records the release before it, whose facts exist only after that release's run: a sentence at the start of the Published line of [`docs/status.md`](../status.md), with the tag, its commit, the run and the GitHub release; that version on the row of the specification it added, in the same file; and a section of [TDD-0021](../tdd/0021-open-source-readiness.md) with the run and what the registries showed. No document names the latest release; GitHub Releases does (SPEC-0021 P10, D-rel-3).
2. A maintainer pushes the tag `vX.Y.Z` on the merge commit. A Claude Code session on the web could not push the tag of 0.1.3, so the owner pushed it; from 0.1.4 on, a local session pushes the tag on the owner's authorization: `git tag -a vX.Y.Z <merge commit> -m "Orchvia X.Y.Z"`, then `git push origin vX.Y.Z`. A version with a suffix, such as `v0.2.0-rc.1`, is a pre-release: npm tag `next`, PyPI pre-release, GitHub pre-release.
