...
2. A maintainer pushes the tag `vX.Y.Z` on the merge commit. A Claude Code session on the web could not push the tag of 0.1.3, so the owner pushed it; a local session pushed the tags of 0.1.4, 0.1.5 and 0.1.6 on the owner's authorization: `git tag -a vX.Y.Z <merge commit> -m "Orchvia X.Y.Z"`, then `git push origin vX.Y.Z`. A version with a suffix, such as `v0.2.0-rc.1`, is a pre-release: npm tag `next`, PyPI pre-release, GitHub pre-release.
