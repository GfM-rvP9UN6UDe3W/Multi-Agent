...
Every usage observation from a runtime is stored with one `usage.recorded` event in the same transaction. The record names its session, model and root task, and the event carries the token counts, so that `usage.summary`, `usage.byTask` and a host's own totals need no other read. A runtime that splits its cache writes into those that live five minutes and an hour reports both counts, and the totals sum them. A Claude dispatch has a record for its main loop and, for each model, one for its calls outside it, such as a compaction. Missing values stay unknown; cost estimates from registered prices are estimates, not bills.
