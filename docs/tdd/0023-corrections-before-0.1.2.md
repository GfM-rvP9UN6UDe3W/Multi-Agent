...
The pull request CI of `known-issues` ([35956615388](https://github.com/masonlee39/orchvia/actions/runs/35956615388)) then failed `A2 Claude retains terminal evidence until delayed process exit confirms stop` on macOS 14 with Node 22: the last event was not the `unknown` error that the test expects. Its held child ignores SIGTERM, and P04's group SIGKILL at the end of the 10 ms cleanup window races the cleanup deadline. On that runner the child's exit was observed first, so the adapter confirmed a stop that the test says it cannot confirm. Logging every group signal across the Claude test files, one file at a time, showed eight tests that receive a group SIGKILL: P04's own; `AC-I03 interrupt timeout retains a late terminal and its original usage identity`, whose assertions do not depend on the child; and six whose child must outlive its cleanup: this one, `AC-R04.3 an uncooperative child keeps cleanup unknown after the bounded fallback`, `AC-R04 close and iterator completion keep a live child held until its observed exit`, `AC-R04 an owned spawn failure is observed without treating a later process error as exit`, `AC-P05 a positive remote observer does not replace local process exit` and `AC-P06 cleanup uncertainty and late terminal retain usage with unknown fields`. They had passed because the adapter usually decides before the exit arrives. A test-only preload, not committed, that turns the adapter's first group SIGTERM into SIGKILL, so that the exit is observed before the deadline, failed all six; without it they passed.

The six now use `refuseGroupSignals(t)`, and pass with and without that preload; afterwards only P04's own test receives a group SIGKILL. AC-P05 and AC-P06 released their held child and then waited a fixed 30 ms before they closed the adapter, a gap that P04's SIGKILL had hidden by ending the child earlier. With only the fixture, 3 of 10 runs of the five files at once, next to four busy loops on four cores, failed that close with `cleanup unconfirmed`, and the unchanged files failed it too under the same load. Both tests now wait until the adapter observes the exit, with a 5-second deadline that only reports a child that never exits. In two rounds of 10 such runs, 19 of 20 passed after the change and 18 of 20 before it. The one failure after it was AC-R04.3's check that its cooperative child had exited within the last 100 ms of the window, which the change does not touch; that file alone passed 20 of 20 runs at once in both versions.
