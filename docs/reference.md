...
| Host queries | `tasks.list` by status and newest first, `tasks.getMany` for up to 100 tasks, `usage.summary` of a root task's tree per model, all through indexes; `blockedBy` says why a waiting task waits |
| Host lifecycle | `close({ mode: 'pause' })` pauses what it interrupts as `owner_shutdown`; `rules.retire` retires a runtime rule; `createEngine` writes its reserve without blocking the event loop |
