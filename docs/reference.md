...
| Host queries | `tasks.list` by status and newest first, `tasks.getMany` for up to 100 tasks, `usage.summary` of a root task's tree and `usage.byTask` of up to 100 tasks per model, all through indexes; `blockedBy` says why a waiting task waits, and `deliveredAt` when a task last delivered |
