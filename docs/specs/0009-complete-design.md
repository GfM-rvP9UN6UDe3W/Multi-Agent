...
| F16 | Full Node/Python/type/format/generated checks; measured 1k/10k/50k retained-task capacity under an explicit 100,000-session limit; finite host/session/queue limits | SPEC-0011's final source revision `597f240` passed all six jobs in [run 35530017913](https://github.com/masonlee39/Multi-Agent/actions/runs/35530017913). SPEC-0012 adds scoped tool-call and default-capacity evidence; its changes need separate verification. The programmatic 10,000-session default is not a 50k production capacity claim |
