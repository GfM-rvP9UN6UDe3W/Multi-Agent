...
- **U03** A rule of the configuration cannot be retired (`VALIDATION_ERROR`). A retired `id` and `version` cannot be registered again (`RULE_RETIRED`); a changed rule takes a new version.
  ...
  Superseded in part by [SPEC-0029](./0029-usage-by-task-and-close-markers.md) C01: registering a retired version again with the same content reactivates it; other content still fails with `RULE_RETIRED`.
