...
import { protocolSchema } from './generated/protocol-schema.ts';
import { OrchestrationError } from './errors.ts';
import type * as WireTypes from './generated/wire.ts';
...
// Supported JSON Schema subset. Audit every node before use; new assertion
// keywords must be implemented explicitly, never silently ignored.
...
export function createWireValidator(document: unknown) {
  ...
  return (name: string, value: unknown) => {
    ...
    if (errors.length)
      throw new OrchestrationError('INVALID_WIRE_DATA', errors.slice(0, 32).join('\n'));
  };
}

export const validateWire = createWireValidator(protocolSchema);
