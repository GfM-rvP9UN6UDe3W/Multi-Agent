...
See the [JSON Schema](schemas/protocol.schema.json) for protocol data structures. Shared wire contracts and integration tests keep the TypeScript and Python types aligned; schema-to-SDK code generation is not implemented.
...
| --- | --- | --- | --- |
| Claude | Optional `@anthropic-ai/claude-agent-sdk` peer dependency; `query()` and native session resume | **0.3.241 is the declared minimum**; package range **`>=0.3.241 <1`**. No exact installed SDK version is locked. | Offline SDK-message fixtures and lifecycle tests. The range is an installation constraint, not proof that every release works. Real SDK/model end-to-end acceptance is pending. |
