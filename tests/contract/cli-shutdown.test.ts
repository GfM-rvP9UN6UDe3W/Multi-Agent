...
// A host exits only after it has durably recorded the stopped turn and closed its database. The
// shutdown budget does not bound those synchronous fsyncs and commits, and each can take hundreds
// of milliseconds on a loaded CI disk. This is a watchdog, not the behavior under test.
const EXIT_WATCHDOG_MS = 30_000;
// A host answers requests only between those synchronous writes, for example while a turn that
// finished after an incomplete shutdown persists its result. This also only catches a silent host.
const RPC_WATCHDOG_MS = 10_000;
...
async function until<T>(
  ...
  throw new Error(
    `Fixture condition did not become true before its deadline${diagnostics ? `: ${diagnostics()}` : ''}`,
  );
}
  ...
  return {
    ...
    async waitForExit() {
      await until(
        ...
        EXIT_WATCHDOG_MS,
        () => `host did not exit; stderr: ${stderr}`,
      );
...
test('0011-R09 slow durable writes after SIGTERM do not fail a successful stdio shutdown', async (t) => {
  const slowSync = new URL('../fixtures/slow-durable-sync.ts', import.meta.url).href;
  const f = await runningHost(t, 'stdio', { mode: 'interrupt', timeoutMs: 500 }, 5000, 0, [
    '--import',
    slowSync,
  ]);
  f.proc.kill('SIGTERM');
  ...
  const persisted = f.persisted();
  assert.equal(persisted.task.status, 'paused');
  assert.equal(persisted.task.reason, 'runtime_interrupted');
  assert.equal(persisted.waits.length, 1);
  assert.equal(persisted.waits[0].data.timeoutMs, 500);
  assert.doesNotMatch(f.diagnostics(), /SHUTDOWN_INCOMPLETE/);
});

for (const transport of ['unix', 'stdio'] as const) {
  for (const signal of ['SIGINT', 'SIGTERM'] as const) {
    for (const mode of ['drain', 'interrupt'] as const) {
      test(`AC-R02 ${transport} ${signal} honors configured ${mode} mode and budget`, async (t) => {
  ...
  test(`AC-R02 ${transport} preserves its omitted shutdown defaults`, async (t) => {
  ...
  test(`AC-R02 ${transport} incomplete drain keeps the host and operation for another signal`, async (t) => {
...
test('AC-R02 stdio owner can continue a signal-started drain through the retained wire', async (t) => {
...
test('AC-R02 owner EOF still uses bounded interrupt despite configured drain and zero timeout', async (t) => {
